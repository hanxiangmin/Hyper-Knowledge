...
def create_llm(
    ...
    **kwargs: Any,
    ...
    if provider in ANTHROPIC_PROVIDERS:
        try:
            ...
            raise ImportError(
                ...
                "Install it with: pip install 'hyperknowledge[anthropic]'"
            ) from e
    ...
    # by default so Hyper-Knowledge's structured extraction works out of the box.
...
def get_client(
    ...
    Backward-compatible: reads ~/.hk/config.toml.

    Args:
        config_path: Config file path, default ~/.hk/config.toml
