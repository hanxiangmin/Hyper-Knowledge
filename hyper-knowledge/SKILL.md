...
3. For diagnosis, run `<launcher> skill doctor --scope user --deep --json` or the corresponding project scope. A non-zero exit is a failed preflight. `outdated` means the installed instructions differ from the current bundled content, even if package versions match; update only when authorized. `drifted` means local edits require inspection before replacement. Managed updates retain a backup.
