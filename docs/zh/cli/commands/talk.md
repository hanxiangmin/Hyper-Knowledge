...
```bash
hk talk ./paper_kb/ -q "总结本文的主要贡献"
```

### 法律分析

```bash
hk talk ./contract_kb/ -q "终止条件是什么？"
```

### 历史研究

```bash
hk talk ./bio_kb/ -q "哪些事件导致了苏轼的政治生涯坎坷？"
```
...
- 尝试重新表述您的问题
- 使用 `-n 10` 增加上下文
- 验证知识库有相关数据：`hk info ./output/`

---

## 另请参见

- [`hk search`](search.md) — 搜索特定项目
- [`hk build-index`](build-index.md) — 构建搜索索引
- [`hk show`](show.md) — 可视化知识图谱
