...
Hyper-Knowledge 提供了一个 [MCP](https://modelcontextprotocol.io) 服务器，让支持 MCP 的助手（Claude Desktop、IDE 智能体等）通过 Model Context Protocol **查询和导出你的知识摘要**。
...
服务器从 `~/.hk/config.toml` 读取 LLM/嵌入器配置——与 CLI 相同，因此请先运行 `hk config init ...`（见 [配置](cli/configuration.md)）。

---

## 接入 MCP 客户端

让你的 MCP 客户端指向 `hk-mcp` 命令。以 Claude Desktop 风格的配置为例：
...
所有工具都接受一个 `ka_path`（由 `hk parse` 创建的目录）。`search`/`ask` 需要索引——用 [`hk build-index`](cli/commands/build-index.md) 构建。

> `export_obsidian` 依赖 Obsidian 导出功能（见 [`hk export obsidian`](cli/commands/export.md)）。若不可用，该工具会返回说明信息而不会报错。
